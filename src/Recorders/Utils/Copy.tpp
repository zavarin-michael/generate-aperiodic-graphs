...
template<>
std::unique_ptr<DiskRecorder<Graph>> Copy(const IRecorder<Automata>* from) {
    auto diskRecorder = dynamic_cast<const DiskRecorder<Automata>*>(from);
    if (diskRecorder) {
        return std::make_unique<DiskRecorder<Graph>>(
...
template std::unique_ptr<DiskRecorder<Graph>> Copy(const IRecorder<Automata>* from);
